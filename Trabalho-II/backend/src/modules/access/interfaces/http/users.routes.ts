...
router.get("/", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    if (req.auth!.role !== "admin") {
      return res.status(403).json({ message: "Apenas admin pode listar usuários." });
    }
...
router.post("/", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const created = await userRepository.create({
      ...
      permissions,
      // Usuário criado pelo admin já nasce com e-mail verificado.
...
router.patch("/:id/status", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    if (req.auth!.role !== "admin") {
      return res.status(403).json({ message: "Apenas admin pode ativar/desativar usuários." });
    }

    const parsed = statusSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ message: "Informe { active: boolean }." });
    }

    const targetId = Number(req.params.id);

    if (targetId === req.auth!.userId) {
      return res.status(400).json({ message: "Não é possível desativar a própria conta." });
    }
    ...
    const updated = await userRepository.setActiveStatus(targetId, parsed.data.active);
