...
function describeAction(method: string, path: string): string {
  ...
  if (path.startsWith("/profiles/me/buyer")) return "Atualização do perfil de comprador";
  if (path.startsWith("/profiles/me/seller")) return "Atualização do perfil de vendedor";
  if (path.startsWith("/products") && method === "POST" && !path.match(/\/comments/)) return "Cadastro de produto";
  if (path.match(/^\/products\/\d+$/) && method === "DELETE") return "Exclusão de produto";
  if (path.match(/^\/comments\/\d+\/like/)) return method === "POST" ? "Curtida em comentário" : "Remoção de curtida";
  if (path.match(/^\/products\/\d+\/comments/) && method === "POST") return "Criação de comentário";
  if (path.match(/^\/comments\/\d+/) && method === "PATCH") return "Edição de comentário";
  if (path.match(/^\/comments\/\d+/) && method === "DELETE") return "Exclusão de comentário";
