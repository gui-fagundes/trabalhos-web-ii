...
import { issueVerificationCode } from "./EmailVerification";
...
export class AccessUseCases {
  ...
  async register(input: unknown) {
    ...
    await issueVerificationCode(created.id, created.email);

    return {
      user: created,
      message: "Cadastro criado. Verifique o código de confirmação enviado por e-mail antes de fazer login.",
    };
  }
  ...
  async login(input: unknown) {
    ...
    if (!user.active) {
      throw new AppError("Conta desativada. Fale com um administrador.", 403);
    }

    if (!user.emailVerifiedAt) {
      throw new AppError("E-mail ainda não verificado. Confirme o código enviado.", 403);
    }
